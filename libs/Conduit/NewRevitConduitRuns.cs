...
namespace JPMorrow.Revit.ConduitRuns.New
{
    /// <summary>
    ...
    {
        [DataMember]
        private List<int> _conduit_ids { get; set; } = new List<int>();
        public IList<int> ConduitIds { get => _conduit_ids; }
        public IList<ElementId> ConduitElementIds { get => _conduit_ids.Select(x => new ElementId(x)).ToList(); }
        public IList<Element> ConduitElements(Document doc) => _conduit_ids.Select(x => doc.GetElement(new ElementId(x))).ToList();
        public int[] WireIds { get => ConduitIds.ToArray(); }

        [DataMember]
        private List<ConduitFitting> _fitting_ids { get; set; } = new List<ConduitFitting>();
        public IList<ConduitFitting> FittingIds { get => _fitting_ids; }

        [DataMember]
        private List<RunEndpointInfo> _run_endpoints { get; set; } = new List<RunEndpointInfo>();
        public IList<RunEndpointInfo> RunEndpoints { get => _run_endpoints; }
		...
		public string GetSets(Document doc) 
		{
			bool is_empty(Parameter p) => string.IsNullOrWhiteSpace(p.AsString());
			bool is_null(Parameter p) => p == null;

			foreach(var id in ConduitIds) 
			{
				var el = doc.GetElement(new ElementId(id));
				var p = el.LookupParameter("Set(s)");
		...
		public string ToString(Document doc)
		{
			return string.Format(
				"Type: {0}\nFrom: {1}\nTo: {2}\nDiameter: {3}\n Length: {4}\nSegments: {5}",
				ConduitMaterialType, From, To, DiameterStr(doc),
				LengthStr(doc), ConduitIds.Count().ToString());
		}
		...
		public static ConduitRunInfo CombineRuns(
			Document doc, ConduitRunInfo cri, 
			...
			out List<ConduitRunInfo> old_cris)
		{
            ...
            return new ConduitRunInfo(doc, cri, matching_cris);
        }

		// copy constructor for combining runs
		private ConduitRunInfo(Document doc, ConduitRunInfo cri, IEnumerable<ConduitRunInfo> others)
		{
            GeneratingConduitId = cri.GeneratingConduitId;
            _conduit_ids = cri.ConduitIds.ToList();
            _run_endpoints = cri.RunEndpoints.ToList();
            From = cri.From;
            To = cri.To;
            ...
            Length = len;
            FittingBends = GetTotalBends(doc);
            ConduitMaterialType = cri.ConduitMaterialType;
            ReportedWireSizes = cri.ReportedWireSizes;
        }

		// normal constructor
        private ConduitRunInfo(Document doc, RunNetwork rn, int[] jbox_ids)
		{
			GeneratingConduitId = rn.StartId;
			_conduit_ids = new List<int>(rn.RunIds.OrderBy(x => x).ToList());
