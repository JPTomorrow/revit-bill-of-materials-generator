...
namespace JPMorrow.Revit.Labor
{
    ...
    public class LaborExchange
    {
        ...
        public static string DefaultLaborFilePath { get; private set; } = string.Empty;
        private List<LaborEntry> CheckEntries { get; set; }

        private List<LaborItem> _items { get; set; }
        public IList<LaborItem> Items { get => _items; }

        public LaborExchange(string settings_base_path, IEnumerable<LaborEntry> entries = null)
        {
            if (entries == null)
                entries = new List<LaborEntry>();

            if (DefaultLaborFilePath.Equals(string.Empty))
                DefaultLaborFilePath = settings_base_path + @"labor\default_labor_dictionary.json";
