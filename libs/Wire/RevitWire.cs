...
namespace JPMorrow.Revit.Wires
{
    ...
    public struct WireColor
    {
        ...
        // Low Voltage Wire
        public static Dictionary<string, string> LowVoltageWireNameToColor = new Dictionary<string, string>() {
            { Wire.LowVoltageWireNames[0], "White w/ Orange Stripe" },
            { Wire.LowVoltageWireNames[1], "Gray" },
            { Wire.LowVoltageWireNames[2], "White" },
            { Wire.LowVoltageWireNames[3], "White w/ Red Stripe" },
            { Wire.LowVoltageWireNames[4], "White" },
            { Wire.LowVoltageWireNames[5], "White" },
            { Wire.LowVoltageWireNames[6], "White" },
            { Wire.LowVoltageWireNames[7], "Black" },
            { Wire.LowVoltageWireNames[8], "Black" },
            { Wire.LowVoltageWireNames[9], "Blue" },
            { Wire.LowVoltageWireNames[10], "Orange" },
        };


        public static string Get_277V(int c, bool is_nuetral, bool staggered, bool boy, PanelPhase phase)
        {


            if (phase == PanelPhase.Three)
            {

                var c_num = NormalizeCircuitNumberThreePhase(c);
            ...
            else if (phase == PanelPhase.Two)
            {

                var c_num = NormalizeCircuitNumberTwoPhase(c, staggered);
        ...
        public static string Get_120V(int c, bool is_nuetral, bool staggered, PanelPhase phase)
        {
            ...
            else if (phase == PanelPhase.Two)
            {
                ...
                // 0 is black // 1 is red for staggered
                if (c_num == 0)
                    return is_nuetral ? White_Black_Stripe : Black;
                else if (c_num == 1)
                    return is_nuetral ? White_Red_Stripe : Red;
        ...
        public static string GetLowVoltageWireColor(string wire_name)
        {
            bool s = LowVoltageWireNameToColor.TryGetValue(wire_name, out string wire_color);
            if (!s)
                throw new Exception("A valid wire name was not provided: " + wire_name);
            return wire_color;
        }

        public static int NormalizeCircuitNumberThreePhase(int c)
        {
            int cNum = c > 6 ? c % 6 : c;
            return cNum;
        }

        public static int NormalizeCircuitNumberTwoPhase(int c, bool staggered)
        {
            ...
            if (staggered)
            {
                bool isOddOdd(int x)
                {
                    var odds = Enumerable.Range(1, 400).Where(x => isOdd(x)).ToList();
                    return odds.IndexOf(x) != -1 && odds.IndexOf(x) % 2 != 0;
                }

                bool isEvenEven(int x)
                {
                    var evens = Enumerable.Range(1, 400).Where(x => isEven(x)).ToList();
                    return evens.IndexOf(x) != -1 && evens.IndexOf(x) % 2 != 0;
                }

                if (isOddOdd(c) || isEvenEven(c)) return 1;
                else return 0;
            }
            else
            {
                if (isEven(c)) return 1;
                else return 0;
            }
    ...
    public class Wire
    {
        ...
        [DataMember]
        public WireMaterialType WireMaterialType { get; private set; }

        public bool IsNoWireExport(WireType type) => Color.Equals("NO WIRE EXPORT") && WireType == type;

        public static string WireTypeName(WireType type)
        {
            var name = Enum.GetName(typeof(WireType), type);
            name = TextTransform.AddSpacesBetweenCaps(name);
            return name;
        }

        public static IEnumerable<string> WireTypeNames =>
            Enum.GetNames(typeof(WireType)).Select(x => TextTransform.AddSpacesBetweenCaps(x));

        public static WireType GetWireType(string type_name) =>
            (WireType)Enum.Parse(typeof(WireType), string.Join("", type_name.Split(' ')));

        public override string ToString()
        {
            return string.Format("[{0}, {1}, {2}, {3}]", CircuitNumber, Size, Color, Enum.GetName(typeof(WireType), WireType));
        }
        ...
        public static string[] LowVoltageWireNames { get; } = new string[] {
            "18-2SA",
            "18-3S",
            "18-4S",
            "18-2D",
        ...
        public Wire(
            ...
            string color, WireType type, WireMaterialType mat_type)
        {
            Size = size;
            Color = color;
            CircuitNumber = circuit_num;
            WireType = type;
            WireMaterialType = mat_type;
        }
    }
    ...
    public class WireManager
    {
        ...
        /// <summary>
        /// True if the wiremanager has wire to export for the wire pull and wire pull total breakdown
        /// </summary>
        public bool HasWire
        {
            get => MasterWireDict.Any(x => !x.Wire.CircuitNumber.ToLower().Equals("no wire export"));
        }
        ...
        public bool ContainsWire(int[] conduit_ids, Wire wire) => GetWires(conduit_ids) != null && GetWires(conduit_ids).Any(x => x.Equals(wire));
        ...
        public WireManager(WireManager old)
        {
            if (MasterWireDict == null)
                MasterWireDict = new List<HashedWire>();
            MasterWireDict.AddRange(old.MasterWireDict);
        }

        public override string ToString()
        {
            string o = "";
            WireType store_type = WireType.Branch;

            foreach (var w in MasterWireDict.Select(x => x.Wire).OrderBy(x => x.WireType).ThenBy(x => x.Color))
            {
                if (store_type == WireType.Branch) store_type = w.WireType;
                if (w.WireType != store_type)
                {
                    store_type = w.WireType;
                    o += "\n";
                }
                o += w.ToString() + "\n";
            }
        ...
        public IEnumerable<Wire> GetWires(WireType type)
        {
            if (MasterWireDict == null || !MasterWireDict.Any()) return new List<Wire>();
            return MasterWireDict.Where(x => x.Wire.WireType == type).Select(x => x.Wire).ToList();
        }
            ...
            IEnumerable<int> raw_circuits, WireCreationData w_data)
        {
            ...
            if (w_data.IsolatedGround)
            {
                ...
                var wire = new Wire("Common", w_data.Ground, g_color, WireType.Branch, w_data.MaterialType);
        ...
        {
            string h_color = WireColor.Get_120V(c, false, w_data.StaggeredCircuits, w_data.PanelPhase);
            var wire = new Wire(c.ToString(), w_data.Hot, h_color, WireType.Branch, w_data.MaterialType);
            AddWire(ids, wire);
            ...
            {
                var nuet_wire = new Wire("Common", w_data.Nuetral, WireColor.White, WireType.Branch, w_data.MaterialType);
                AddWire(ids, nuet_wire);
            }
            else
            {
                var color = WireColor.Get_120V(c, true, w_data.StaggeredCircuits, w_data.PanelPhase);
                var nuet_wire = new Wire(c.ToString(), w_data.Nuetral, color, WireType.Branch, w_data.MaterialType);
                AddWire(ids, nuet_wire);
            }
        }
        ...
        private void GetWireForBranchCircuit277V(
            int c, int[] ids, WireCreationData w_data)
        {
            string h_color = WireColor.Get_277V(c, false, w_data.StaggeredCircuits, w_data.BOY, w_data.PanelPhase);
            var wire = new Wire(c.ToString(), w_data.Hot, h_color, WireType.Branch, w_data.MaterialType);
            AddWire(ids, wire);
            ...
            if (!w_data.PhaseNuetral)
            {
                var nuet_wire = new Wire("Common", w_data.Nuetral, WireColor.Gray, WireType.Branch, w_data.MaterialType);
                AddWire(ids, nuet_wire);
            }
            else
            {
                var color = WireColor.Get_277V(c, true, w_data.StaggeredCircuits, w_data.BOY, w_data.PanelPhase);
                var nuet_wire = new Wire(c.ToString(), w_data.Nuetral, color, WireType.Branch, w_data.MaterialType);
                AddWire(ids, nuet_wire);
            }
        }
            ...
            bool staggered, bool boy)
        {
            ...
            foreach(Wire wire in wires)
            {
                ...
                {
                    new_wire = new Wire(
                        wire.CircuitNumber, wire.Size, WireColor.Get_120V(c_num, false, staggered), 
                        wire.WireType, wire.WireMaterialType);
                ...
                {
                    new_wire = new Wire(
                        wire.CircuitNumber, wire.Size, WireColor.Get_277V(c_num, false, staggered, boy), 
                        wire.WireType, wire.WireMaterialType);
                    ret_man.AddWire(conduit_ids, new_wire);
                    continue;
                }
                ...
                {
                    new_wire = new Wire(
                        wire.CircuitNumber, wire.Size, WireColor.Get_120V(c_num, true, staggered), 
                        wire.WireType, wire.WireMaterialType);
                    ret_man.AddWire(conduit_ids, new_wire);
                    continue;
                }
                ...
                {
                    new_wire = new Wire(
                        wire.CircuitNumber, wire.Size, WireColor.Get_277V(c_num, true, staggered, boy), 
                        wire.WireType, wire.WireMaterialType);
                    ret_man.AddWire(conduit_ids, new_wire);
                    continue;
                }
    ...
    public class WireCreationData
    {
        ...
        public WireMaterialType MaterialType { get; private set; }
        public PanelPhase PanelPhase { get; private set; }


        public WireCreationData(
            string hot, string nuetral, string ground,
            bool stag, bool boy, bool phase_nuet,
            bool iso_grd, string mat_type, string panel_phase)
        {
            Hot = hot;
            Nuetral = nuetral;
            Ground = ground;
            ...
            MaterialType = GetMaterialTypeFromString(mat_type);
            PanelPhase = (PanelPhase)int.Parse(panel_phase);
        }

        public static WireMaterialType GetMaterialTypeFromString(string mat_type)
        {
            if (mat_type.Equals("Copper"))
                return WireMaterialType.Copper;
            else if (mat_type.Equals("Aluminium"))
                return WireMaterialType.Aluminium;
            else
                return WireMaterialType.Special;
        }
    }
